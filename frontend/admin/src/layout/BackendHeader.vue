...
<div class="pb-1 mt-2 menudivider d-md-none"></div>
 <ul class="navbar-nav ms-auto m-0 p-0 d-md-none d-lg-none" v-for="(d, index) in mainMenu" :key="index">
        <li class="nav-item m-3 mb-1 mt-1 mb-md-0 mt-md-0 me-md-2"> <a :href="'/app/secure/'+d['menuName']" class="link-dark nav-link"> {{d['menu_description']}} </a></li>
</ul>
<ul class="navbar-nav m-0 p-0 ms-auto">
        ...
        <li class="nav-item m-3 mb-md-0 mt-md-0 me-md-3"> <a v-bind:class="'gradient_txt' + appinfo.f_color" href="/app/site/contactus"><i class="bi-question-circle" role="img" aria-label="Help" style="font-size: 1.2rem;"></i></a> </li>
    </ul>
</div>
        ...
        <p class="lead m-0 p-0 text-primary">Administrator</p> 
        <big>{{usersession['surname']}} </big> <br>
        <small>{{usersession['email_one']}} </small>
        </div>
        ...
        <li class="mb-2">
        <p class="pb-0 mb-1 align-items-center rounded collapsed" data-bs-toggle="collapse" data-bs-target="#chats" aria-expanded="false">
        <i class="bi-chat me-1"></i>    Chats
        <i class="bi-chevron-down  float-end"></i>
        </p>
        <div class="collapse" id="chats">
        <ul class="btn-toggle-nav list-unstyled fw-normal pb-1 small" v-for="(d, index) in chats" :key="index">
        <li> <a :href="'/app/secure/'+d['menuName']" class="link-dark nav-link"><i class="bi-dash"></i> {{d['menu_description']}} </a></li>
        </ul>
        </div>
        </li>  
        </ul>
    ...
    <div class="border-top"></div>
        ...
        <i class="bi-gear me-1"></i>    Admin
        <i class="bi-chevron-down float-end"></i>
        </p>
        <div class="collapse" id="admin">
        <ul class="btn-toggle-nav list-unstyled fw-normal pb-1 small" v-for="(d, index) in admin_menu" :key="index">
        <li> <a :href="'/app/secure/'+d['menuName']" class="link-dark nav-link"><i class="bi-dash"></i> {{d['menu_description']}} </a></li>
        </ul>
        </div>
        </li>  
...
export default {
        ...
        mainMenu: [
            ...
            {"menuName":'history', 'menu_description':'History'},
            ],
        chats: [
            {"menuName":'history', 'menu_description':'Chats'},
            ],
        admin_menu: [
            {"menuName":'adminaccount', 'menu_description':'Admin account'},
        ],
        webcontent:[],
        users:[],
        settingMenu:[],
            ...
            window.location='/app/site/logout'
                ...
                this.isSession = true
                   this.usersession = this.$session.get('usersession')
                    this.passport = this.usersession['email_one'].toLowerCase()+'.png'
                }else{
                    window.location='/app/site/logout'
                }
            }
