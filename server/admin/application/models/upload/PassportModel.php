...
class PassportModel extends CI_Model{
...
function update($data, $param, $update){
    ...
    if ($res->num_rows() > 0) {
        $this->db->where('applicant_number', $param['userid'])
                    ->update('applicants_passport', $update);

        if ($this->db->affected_rows() > 0) {
            $response=array( 
                'status' => 'success',
                'statusmsg' => 'success',
                'msg' => 'Your Passport was updated successfully, click on the next button to continue.',
        ...
        $this->db->insert('applicants_passport', $data);
         if ($this->db->affected_rows() > 0) {
            $response=array( 
                'status' => 'success',
                'statusmsg' => 'success',
                'msg' => 'Your Passport was uploaded successfully, click on the next button to continue.',
                'classname' => 'alert-primary',
                'result' => '',  
                'redirect' => '',
            );
