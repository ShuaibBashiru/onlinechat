...
class SignAdminModel extends CI_Model{
...
function SignIn($data){
    try{
    $sql = "SELECT * FROM admin_record WHERE email_one=? AND pwd_auth=? LIMIT 1";
    $res = $this->db->query($sql, array($data['userid'], $data['password']));
    if ($res->num_rows() > 0) {
        $rows=$res->row();
